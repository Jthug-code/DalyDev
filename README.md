...
```bash
cd exo-mcp  # clone this repository
uv sync
```
...
- `/ABSOLUTE/PATH/TO/exo-mcp` with your repo path
...
If the remote repository does not exist yet (on your machine, with `gh` authenticated):
...
```bash
gh repo create Jthug-code/exo-mcp --public --source . --remote origin --push
```
